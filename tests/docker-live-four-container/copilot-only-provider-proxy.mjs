...
const upstreamValue = process.env.AGENT_MULTIPLEX_COPILOT_ONLY_PROXY_UPSTREAM;
if (!bindHost || !readyFile || !upstreamValue) {
  throw new Error(
    "usage: AGENT_MULTIPLEX_COPILOT_ONLY_PROXY_UPSTREAM=<url> " +
      "copilot-only-provider-proxy.mjs <bind-host> <ready-file>",
  );
}
