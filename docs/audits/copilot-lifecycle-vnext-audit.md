...
| p2prpc authenticated-session retirement is break/reconnect rather than seamless renewal | **Confirmed for 0.2.1; released renewal pinned in current source.** | Supplied stability evidence recorded the 900-second boundary. Independent core `0.3.0-renewal.0` at `ca7bb6fb7b791813c937ddbf9bde62423d097373` publishes renewal contract 1; the branch pins its registry integrity and requires application contract `6.renewal.1`. The earlier local candidate evidence does not qualify the final framework/consumer graph. |
