...
name="agent-multiplex-luna-$RANDOM-$$"
[[ -r "$key" && -r "$config" && -s "$ca" && -f "$repo/packages/adapter-copilot/test/luna-resume-smoke.mjs" ]] || { echo 'Luna smoke preflight missing input' >&2; exit 2; }
...
timeout --signal=TERM --kill-after=10s 240s docker run --rm --name "$name" \
  ...
  node --import tsx packages/adapter-copilot/test/luna-resume-smoke.mjs
