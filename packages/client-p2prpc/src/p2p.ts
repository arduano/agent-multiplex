 ...
 * enrollment and the returned protocol-v6 control-node descriptor are both
    ...
    const establish = (): Promise<ConnectedControlNodeSource> => {
      connecting ??= (async () => {
        ...
        if (!composite.ingress?.gateways?.enroll || !composite.access) {
          throw new Error("pinned peer is not a protocol-v6 composite control node");
        }
        ...
        if (
          enrollment.canonical.protocolVersion !== 6 ||
        ...
        if (
          description.protocolVersion !== 6 ||
          description.componentKind !== "control-node" ||
          description.dataAuthority !== "control-node"
        ) {
          throw new Error("pinned peer is not a protocol-v6 control-node source");
        }
